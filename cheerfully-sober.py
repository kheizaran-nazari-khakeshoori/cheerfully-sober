...
tk.Label(root, text="Sex:").grid(row=2, column=0, sticky="e")
sex_var = tk.StringVar(value="male")
tk.OptionMenu(root, sex_var, "male", "female").grid(row=2, column=1)

tk.Label(root, text="Drink volume (ml):").grid(row=3, column=0, sticky="e")
drink_volume_entry = tk.Entry(root)
drink_volume_entry.grid(row=3, column=1)

tk.Label(root, text="Drink ABV (%):").grid(row=4, column=0, sticky="e")
drink_abv_entry = tk.Entry(root)
drink_abv_entry.grid(row=4, column=1)

tk.Label(root, text="Hours since first drink:").grid(row=5, column=0, sticky="e")
hours_entry = tk.Entry(root)
hours_entry.grid(row=5, column=1)

# Calculate button
tk.Button(root, text="Calculate BAC", command=calculate_bac).grid(row=6, column=0, columnspan=2, pady=10)
